...
def helpMessage() {
    ...
    Generic:
      ...
      --coldata                     CSV file with first column matching files basename and other columns with metadata for each sample (treatment, group, etc)
...
if (params.coldata){
  coldata_ch = Channel
        .fromPath(params.coldata, checkIfExists: true)
        .ifEmpty { exit 1, "coldata annotation file not found: ${params.coldata}" }
} else {
  coldata_ch = Channel.empty()
}
